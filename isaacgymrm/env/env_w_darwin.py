...
class RoboMasterEnv:
    ...
    def create_envs(self):
        ...
        # Create darwin asset
        darwin_asset_file = "darwin/darwin.urdf"
        darwin_options = gymapi.AssetOptions()
        darwin_options.angular_damping = 0.01
        darwin_options.max_angular_velocity = 100.0
        darwin_options.fix_base_link = False
        darwin_options.disable_gravity = False
        darwin_options.default_dof_drive_mode = gymapi.DOF_MODE_EFFORT
        darwin_asset = self.gym.load_asset(self.sim, asset_root, darwin_asset_file, darwin_options)
        ...
        self.darwin_torso_index = 0
        self.darwin_num_bodies = self.gym.get_asset_rigid_body_count(darwin_asset)
        self.darwin_num_dof = self.gym.get_asset_dof_count(darwin_asset)
        self.darwin_num_joints = self.gym.get_asset_joint_count(darwin_asset)

        darwin_start_pose_0 = gymapi.Transform()
        darwin_start_pose_0.p = gymapi.Vec3(-3, 0, 0.01)
        darwin_start_pose_0.r = gymapi.Quat(0.0, 0.0, 0.0, 1.0)

        darwin_start_pose_1 = gymapi.Transform()
        darwin_start_pose_1.p = gymapi.Vec3(3, 0, 0.01)
        darwin_start_pose_1.r = gymapi.Quat(0.0, 0.0, 1.0, 0.0)

        # self.start_rotation = torch.tensor([darwin_start_pose.r.x, darwin_start_pose.r.y, darwin_start_pose.r.z, darwin_start_pose.r.w], device=self.args.sim_device)
        ...
        for i in range(self.num_env):
            ...
            # Create darwin actors
            darwin_handle_0 = self.gym.create_actor(env_ptr, darwin_asset, darwin_start_pose_0, "darwin0", i, 1, 0)
            darwin_handle_1 = self.gym.create_actor(env_ptr, darwin_asset, darwin_start_pose_1, "darwin1", i, 1, 0)
        
            self.gym.enable_actor_dof_force_sensors(env_ptr, darwin_handle_0)
            self.gym.enable_actor_dof_force_sensors(env_ptr, darwin_handle_1)
            self.darwin_handles.append(darwin_handle_0)
            self.darwin_handles.append(darwin_handle_1)

            for j in range(self.darwin_num_bodies):
                self.gym.set_rigid_body_color(
                    env_ptr, darwin_handle_0, j, gymapi.MESH_VISUAL, gymapi.Vec3(1.0, 0, 0))
                self.gym.set_rigid_body_color(
                    env_ptr, darwin_handle_1, j, gymapi.MESH_VISUAL, gymapi.Vec3(0.0, 0, 1.0))
